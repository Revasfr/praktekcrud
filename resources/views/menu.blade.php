     ...
     <div class="container mt-4">
        <div class="card">
            <div class="card-header">
                <h4 class="float-start">Daftar Menu</h4>
                <a href="/tambah_menu" class="btn btn-primary float-end">Tambah</a>
            </div>
            <div class="card-body">
...
<tbody>
@foreach ($menu as $menukopi)
    <tr>
        <th scope="row">{{$loop->iteration}}</th>
        <td>{{$menukopi->menu}}</td>
        <td>{{$menukopi->harga}}</td>
        <td><a href="/edit_menu/{{$menukopi->id_kopi}}" class="btn btn-warning">Edit</a>
            <a href="/delete_menu/{{$menukopi->id_kopi}}" class="btn btn-danger" onclick="return confirm('Yakin ingin menghapus data ?')">Delete</a>
        </td>
    </tr>
    @endforeach
</tbody>
